...
Interpolation gets rid of unwanted peaks in your signal. Interpolations are calculated across one or more regions set by the user using the :py:meth:`interpolate() <ramcoh:ramCOH.raman.baseclass.RamanProcessing.interpolate>` method from `ramCOH <https://ramcoh.readthedocs.io/en/latest>`_.
The interpolation algorithm is similar to the one used for calculating :ref:`baselines </baseline_correction.rst>` and interpolation settings are adjusted similarly as well. 
...
Smoothing of the interpolated signal is set in the same way as :ref:`baseline smoothing </baseline_correction.rst#baseline-smoothing>`.
