...
class settings:
    ...
    def open_general_settings(self):
        ...
        ttk.Label(window, text="File sample name separator", font=font).grid(
            row=3, column=0
        )
        ...
        ttk.Label(window, textvariable=self.name_separator_var, font=font).grid(
            row=3, column=1
        )
        ttk.Label(
            window,
            text="Press Enter to store\nReload spectra to use new laser wavelength",
            font=font,
        ).grid(row=4, column=0, columnspan=3, sticky=("nesw"))
        # Create entry fields
    ...
    def open_bir_settings(self):
        ...
        birs_title = ttk.Label(
            text="Baseline interpolation regions", font=(self.app.font, "14", "bold")
        )
        bir_label_frame = ttk.Labelframe(
            window, text="Silicate interpolation\nregions", padding=padding
        )
        ...
        for i in range(bir_number):
            ...
            bir_frame.grid(
                row=int(i / column_max), column=int(i % column_max), sticky=("nesw")
            )
            bir_frames.append(bir_frame)

        for i, (name, bir) in enumerate(birs.items()):
            ttk.Label(
                bir_frames[i], text=name, font=(self.app.font, "14", "bold")
            ).grid(row=0, column=0, sticky=("nesw"))
            ttk.Separator(bir_frames[i], orient=tk.HORIZONTAL).grid(
            ...
            for j, region in enumerate(bir):
                ...
                if j + 1 == len(bir):
                    text = f"{region[0]: <4} \u2192"
                ttk.Label(bir_frames[i], text=text, font="TkFixedFont").grid(
                    row=j + 1, column=0, sticky=("nesw")
                )
        ...
        ttk.Label(table_frame, text="BIR", font=(self.app.font, "14", "bold")).grid(
            row=0, column=0, sticky=("nws")
        )
        ttk.Label(table_frame, text="Use", font=(self.app.font, "14", "bold")).grid(
            row=0, column=1, sticky=("nesw")
        )
        ttk.Label(table_frame, text="Default", font=(self.app.font, "14", "bold")).grid(
            row=0, column=2, sticky=("nesw")
        )
        for i, name in enumerate(birs):
            ttk.Label(table_frame, text=name, font=font).grid(
                row=i + 1, column=0, sticky=("nes")
            )
            tick_var = tk.BooleanVar()
            tick_var.set(self.birs_use[name])
            tick = ttk.Checkbutton(
                table_frame, variable=tick_var, onvalue=True, offvalue=False
            )
        ...
        ttk.Separator(table_frame, orient=tk.VERTICAL).grid(
            row=1, column=1, rowspan=len(birs), sticky=("nse")
        )
